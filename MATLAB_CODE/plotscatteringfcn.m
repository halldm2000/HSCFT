function [krInterp,srSmoothed,kr,sr]=plotscatteringfcn2(fieldname,timeStep)
...
clear sr;
for (i=1:nbins)
    mask = ones(nx,ny).*exp(-(k-kr(i)).^2/(0.5*deltaK).^2); 
...
figure(5); plot(krInterp,srSmoothed,'k-'); %title('Scaled Radial Scattering Function vs wavenumber');
axis tight; xlim([0 0.6]); ylim([0 0.04]); hold on;
