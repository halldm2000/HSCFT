function isophis(which,timeStep,rep)
filename='phi0'
...
for j=1:length(which)
    ...
    diff(:,:,:) = phi(:,:,:,target)-maxOther(:,:,:);
    rgb = hsv2rgb([(2/3)*(target-1)/(numLiquids-1) 1 1 ]);
    p1 = patch(isosurface(diff,0,diff),'FaceColor',rgb,'EdgeColor','none');
    p2 = patch(isocaps(diff,0),'FaceColor',rgb,'EdgeColor','none');
    isonormals(diff,p1);
end;
